...
class dbfunctions:
    ...
    def add_expenses(self,  userID, description, amount,origin): 
        ...
        if transaction_idcheck == None: 
            transaction_id = 1 
            dbcursor.execute(f"INSERT INTO expenses (userID, username, transaction_id, expense_description, amount,origin) VALUES (%s, %s,%s,%s,%s,%s)" , (userID, self.username, transaction_id, description, amount,origin))
            dbconn.commit()
        else: 
    ...
    def multischedule(self, userID, paymentname, amount, duedate, scheduletype, months,enddate):
        ...
        dbcursor.execute(f"SELECT scheduleid from scheduledpayments WHERE username = %s", (self.username, ))
        scheduleid = dbcursor.fetchall()
        print(f"[SCHEDULEID = {scheduleid}]")

        if len(scheduleid) == 0: 
            print(f"SCHEDULEID == 0")
            scheduleid = 1
            ...
            #work on semi monthly
                    ...
                    for i in range(months): 
                        ...
                        print(f"[FOR LOOP | [1] processed_date] {processed_date}")
                        ...
                        dbcursor.execute(f"INSERT INTO scheduledpayments(userID, username, scheduleid, paymentname, amount, duedate) VALUES(%s,%s,%s,%s,%s,%s)",(userID,self.username,scheduleid,paymentname, amount, processed_date))
                        dbconn.commit()
                        startdate = datetime.strptime(processed_date, "%Y-%m-%d").date() + timedelta(weeks = 4)
                        scheduleid = scheduleid + 1
                ...
                for i in range(months):
                    processed_date = startdate
                    processed_date = f"{processed_date.year}-{processed_date.month}-{start_day}"
                    dbcursor.execute(f"INSERT INTO scheduledpayments(userID, username, scheduleid, paymentname, amount,duedate) VALUES(%s,%s,%s,%s,%s,%s)", (userID,self.username,scheduleid,paymentname, amount, processed_date))
                    dbconn.commit()
                    startdate = datetime.strptime(processed_date, "%Y-%m-%d").date() + timedelta(weeks = 4)
                    scheduleid = scheduleid + 1
...
def addremove_expense(): 
    ...
    if option == "add": 
        ...
        dbrequest.add_expenses(userid, expense_descriptionweb,expense_amountweb, origin) 
...
@app.route('/database_handler', methods = ['GET'])

def database_handler():
    user = request.cookies.get('username')
    dbconn = dbconnection()
    dbcursor = dbconn.cursor() 
    dbcursor.execute(f"SELECT * FROM scheduledpayments WHERE username = %s", (user,))
    schedule_list = dbcursor.fetchall()
    date_now = date.today()
    print(f"\n [DATE NOW] {date_now} \n \n")
    origin = "autosched"
    print(f"[SCHEDULE LIST]: {schedule_list} \n \n")

    print(f"[DATE TODAY] {date_now}")

    if range(len(schedule_list)) == 0:
        pass
    else:
        for i in range(len(schedule_list)):
            if schedule_list[i][5] <= date_now:
                print(f"[SCHEDULED TODAY]: {schedule_list[i][5]}")
                #check if the scheduled is already in database
                fetch_expense = fetchdb(user)
                result = fetch_expense.expenses()

                
                dbrequest = dbfunctions(user)
                dbrequest.add_expenses(schedule_list[i][0], schedule_list[i][3], schedule_list[i][4],origin)
                print(f"\n[REMOVING...] {schedule_list[i]}\n")
                dbcursor.execute(f"DELETE FROM scheduledpayments WHERE scheduleID = %s", (schedule_list[i][2], ))
                dbconn.commit()
                print(f"[REMOVED]")
